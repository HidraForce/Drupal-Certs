...
@Component({
  ...
  @if(stage()==='setup'){
    <section class="setup-card framed"><span class="big-icon">⚙</span><span class="eyebrow">EXAMINATION CHAMBER</span><h1>Calibrate the machine</h1><p>Select the certification mechanism, question count, and total examination time.</p>
      ...
      <label>Number of questions<select [(ngModel)]="count"><option [ngValue]="5">5 · Brief inspection</option><option [ngValue]="10">10 · Standard trial</option><option [ngValue]="20">20 · Master examination</option><option [ngValue]="40">40 · Extended examination</option><option [ngValue]="60">60 · Expert examination</option><option [ngValue]="80">80 · Full certification trial</option></select></label>
      <label>Total quiz time (minutes)<input [(ngModel)]="totalMinutes" type="number" min="1" max="240" step="1"></label>
      <p class="timer-preview">Each question receives approximately <strong>{{perQuestionPreview()}}</strong>.</p>
      <button class="primary wide" (click)="start()">Engage mechanism</button>
    </section>
  } @else if(stage()==='loading'){<section class="setup-card framed loading-card" aria-live="polite"><span class="loading-gears"><i>⚙</i><i>⚙</i></span><h2>Loading questions…</h2><p>Opening the {{trackLabel()}} question vault. This may take a moment.</p><div class="loading-track"><i></i></div></section>
  } @else if(stage()==='playing'&&current;as q){
    <section class="game-top"><div><span>{{trackLabel()}} · Question {{index()+1}} of {{questions().length}}</span><div class="progress"><i [style.width.%]="((index()+1)/questions().length)*100"></i></div></div><div class="timer" [class.danger]="seconds()<=10">◷ {{timerText()}}</div></section>
...
export class PlayComponent implements OnDestroy {
  ...
  stage=signal<'setup'|'loading'|'playing'|'done'|'error'|'exhausted'>('setup');count=5;totalMinutes=40;certification:'frontend'|'backend'|'devops'='frontend';
  questions=signal<Question[]>([]);index=signal(0);selected=signal<number|null>(null);result=signal<Result|null>(null);score=signal(0);seconds=signal(30);reported=signal(false);reportMessage=signal('');private timer?:number;private secondsPerQuestion=30;
  ...
  perQuestionPreview(){return this.formatDuration(this.calculateSecondsPerQuestion());}
  timerText(){return this.formatDuration(this.seconds());}
  async start(){this.totalMinutes=Math.min(240,Math.max(1,Number(this.totalMinutes)||1));this.secondsPerQuestion=this.calculateSecondsPerQuestion();this.seconds.set(this.secondsPerQuestion);this.stage.set('loading');const selectedTrack=this.certification;try{const token=await this.auth.token();this.http.get<Question[]>(`${this.config.quizApi}/v1/questions?certification=${encodeURIComponent(selectedTrack)}`,{headers:{Authorization:`Bearer ${token}`},observe:'response'}).subscribe({next:response=>{const available=(response.body||[]).filter(question=>question.certification===selectedTrack);this.questions.set([...available].sort(()=>Math.random()-.5).slice(0,Math.min(this.count,available.length)));if(!this.questions().length){this.stage.set(response.headers.get('X-Question-Status')==='exhausted'?'exhausted':'error');return;}this.stage.set('playing');this.startTimer();},error:()=>this.stage.set('error')});}catch{this.stage.set('error');}}
  ...
  next(){if(this.index()+1>=this.questions().length){void this.finish();return;}this.index.update(v=>v+1);this.selected.set(null);this.result.set(null);this.reported.set(false);this.reportMessage.set('');this.seconds.set(this.secondsPerQuestion);this.startTimer();}
  restart(){clearInterval(this.timer);this.stage.set('setup');this.index.set(0);this.score.set(0);this.selected.set(null);this.result.set(null);this.reported.set(false);this.reportMessage.set('');this.secondsPerQuestion=this.calculateSecondsPerQuestion();this.seconds.set(this.secondsPerQuestion);}
  ...
  private calculateSecondsPerQuestion(){return Math.max(1,Math.round((Math.min(240,Math.max(1,Number(this.totalMinutes)||1))*60)/Math.max(1,this.count)));}
  private formatDuration(value:number){const minutes=Math.floor(value/60),seconds=value%60;return minutes?`${minutes}:${String(seconds).padStart(2,'0')} min`:`${seconds} sec`;}
